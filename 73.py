...
def is_symmetric(A: Matrix, tol=1e-10) -> bool:
    if A.rows != A.cols:
        return False
...
def run_all_tests(n, A, x_true_data):
    ...
    try:
        ...
        cond_chol = A.condition_number('2')

        print("x_calculated =", x_chol)
        print("x_exact =", x_true)
        print("diff =", diff_chol)
        print("cond =", cond_chol)
...
def compare_cholesky_theory():
    ...
    print(f"Отношение Гаусс/Холецкий: {gauss_ops / chol_total:.2f}x")
